...
exports.ItemCategory = exports.ServiceType = exports.HotelService = void 0;
...
(function (ItemCategory) {
    ...
    ItemCategory["MAIN_COURSE"] = "main_course";
...
const HotelServiceSchema = new mongoose_1.Schema({
    hotelId: {
        type: mongoose_1.Schema.Types.ObjectId,
...
HotelServiceSchema.pre('validate', function (next) {
    ...
    for (const item of this.menuItems) {
        if (!allowedCategories.includes(item.category)) {
            this.invalidate('menuItems', `Category "${item.category}" is not valid for service type "${serviceType}"`);
            break;
        }
    }
    next();
...
// Instance methods
HotelServiceSchema.methods.isOpen = function (date) {
...
HotelServiceSchema.methods.findItemById = function (itemId) {
    const itemIdStr = itemId.toString();
    return this.menuItems.find((item) => item.itemId.toString() === itemIdStr) || null;
};
HotelServiceSchema.methods.getAvailableItems = function () {
    return this.menuItems.filter((item) => item.available);
};
HotelServiceSchema.methods.getItemsByCategory = function (category) {
    return this.menuItems.filter((item) => item.category === category);
};
// Virtual properties
HotelServiceSchema.virtual('itemCount').get(function () {
    return this.menuItems.length;
});
HotelServiceSchema.virtual('availableItemCount').get(function () {
    return this.menuItems.filter(item => item.available).length;
});
HotelServiceSchema.virtual('categories').get(function () {
    return [...new Set(this.menuItems.map(item => item.category))];
});
// Create and export the model
const HotelService = mongoose_1.default.model('HotelService', HotelServiceSchema);
